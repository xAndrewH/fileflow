...
const STATUS_BORDER: Record<string, string> = {
  idle:             "border-slate-200 dark:border-slate-800/60",
  "loading-ffmpeg": "border-slate-200 dark:border-slate-800/60",
  converting:       "border-slate-200 dark:border-slate-800/60",
  done:             "border-green-200 dark:border-green-900/40",
...
export default function FileCard({ item, onConvert, onRemove, onChange, onOpenKeyModal, dragHandleProps }: Props) {
  ...
  return (
    <div className={`relative bg-slate-50 dark:bg-slate-900/70 border rounded-xl transition-colors duration-300 ${STATUS_BORDER[item.status] ?? "border-slate-200 dark:border-slate-800/60"}`}>
      {/* Colored left status bar */}
      <div className={`absolute left-0 top-0 bottom-0 w-[3px] rounded-l-xl transition-colors duration-300 ${STATUS_BAR[item.status] ?? "bg-slate-200 dark:bg-slate-700/60"}`} />

      <div className="pl-5 pr-4 py-4">
        <div className="flex items-start gap-3">
          {/* Icon / thumbnail */}
          <div className="shrink-0 w-10 h-10 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700/60 overflow-hidden flex items-center justify-center text-lg mt-0.5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0 flex-1">
                <p className="text-slate-900 dark:text-white font-medium text-sm truncate leading-snug" title={item.name}>
                  {item.name}
                </p>
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                {dragHandleProps && (
                  <div
                    {...dragHandleProps}
                    className="p-1.5 text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 cursor-grab active:cursor-grabbing transition-colors rounded"
                ...
                <button
                  ...
                  className="text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 transition-colors p-1 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800"
                >
            ...
            {!isProcessing && !isDone && (
              <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
                {/* Mode toggle */}
                {canCompress && (
                  <div className="flex w-full sm:w-auto rounded-lg overflow-hidden border border-slate-300 dark:border-slate-700/80 text-xs shrink-0">
                    <button
                ...
                {item.mode === "compress" && (
                  <div className="w-full sm:w-auto sm:flex-1 sm:min-w-40 space-y-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-slate-500 dark:text-slate-400 text-xs shrink-0">Quality</span>
                      <input
                        type="range"
                        min={1}
                        max={100}
                      ...
                      />
                      <span className="text-slate-900 dark:text-white text-xs w-8 text-right shrink-0 font-mono">{item.quality}%</span>
                    </div>
            ...
            {needsCloudConvert && !isProcessing && !isDone && !isError && (
              <div className="mt-2.5 flex items-start gap-2 px-3 py-2 rounded-lg bg-yellow-500/10 border border-yellow-500/20">
                <svg className="w-3.5 h-3.5 text-yellow-600 dark:text-yellow-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 5.25a3 3 0 013 3m3 0a6 6 0 01-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H2.25v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1121.75 8.25z" />
                </svg>
                <span className="text-yellow-700 dark:text-yellow-300 text-xs leading-relaxed">
                  ⚡ Requires a free CloudConvert API key · 25 conversions/day free
                  {onOpenKeyModal && (
                    <button onClick={onOpenKeyModal} className="ml-1.5 underline underline-offset-2 hover:text-yellow-700 dark:hover:text-yellow-200 transition-colors">Add key</button>
                  )}
                </span>
              </div>
            )}
            ...
            {item.status === "loading-ffmpeg" && (
              <div className="mt-3" aria-busy={true}>
                <p className="text-blue-600 dark:text-blue-400 text-xs mb-1.5 font-medium">Loading FFmpeg engine…</p>
            ...
            {item.status === "converting" && (
              <div className="mt-3" aria-busy={true}>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-blue-600 dark:text-blue-400 text-xs font-medium">Converting…</span>
                  <span className="text-blue-600 dark:text-blue-400 text-xs font-mono">{item.progress}%</span>
                </div>
                <div className="h-1 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                  <div
            ...
            {isDone && (
              <div className="mt-3 flex items-center gap-3 flex-wrap">
                ...
                <button
                  ...
                  className="text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 text-xs transition-colors"
                >
                  Convert again
                </button>
              </div>
            ...
            {isError && (
              <div className="mt-3 flex flex-wrap items-start gap-3">
                <span className="text-red-600 dark:text-red-400 text-xs break-words min-w-0 flex-1">✗ {item.error ?? "Unknown error"}</span>
