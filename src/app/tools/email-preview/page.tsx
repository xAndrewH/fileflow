  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Email Subject Line Previewer</h1>
          ...
          <div className="flex flex-col gap-4">
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-sm text-slate-500 dark:text-slate-400">Subject Line</label>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-sm text-slate-500 dark:text-slate-400">Preheader Text <span className="text-slate-400 dark:text-slate-600">(optional)</span></label>
                <span className="text-xs text-slate-400 dark:text-slate-600">{preheaderLen}/150</span>
              </div>
              <input
                type="text"
                value={preheader}
            ...
            <div>
              <label className="block text-sm text-slate-500 dark:text-slate-400 mb-1.5">Sender Name</label>
              <input
                type="text"
                value={senderName}
        ...
        <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-5">
          <p className="text-slate-900 dark:text-white text-sm font-semibold mb-4">Analysis</p>
          <div className="flex flex-col gap-3">
            <div className={`flex items-center justify-between p-3 rounded-lg border ${subjectLen > 0 ? subjectBg : "border-slate-200 dark:border-slate-800/40"}`}>
              <span className="text-sm text-slate-700 dark:text-slate-300">Subject length</span>
            ...
            {spamWords.length > 0 && (
              <div className="flex items-start justify-between p-3 rounded-lg border bg-red-500/10 border-red-500/20">
                <span className="text-sm text-slate-700 dark:text-slate-300">Spam trigger words</span>
                <span className="text-sm font-medium text-red-600 dark:text-red-400 text-right">{spamWords.join(", ")}</span>
              </div>
            )}

            <div className="flex items-center justify-between p-3 rounded-lg border border-slate-200 dark:border-slate-800/40">
              <span className="text-sm text-slate-700 dark:text-slate-300">Emoji count</span>
              <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{emojiCount}</span>
            </div>

            <div className="flex items-center justify-between p-3 rounded-lg border border-slate-200 dark:border-slate-800/40">
              <span className="text-sm text-slate-700 dark:text-slate-300">Contains question mark</span>
              <span className={`text-sm font-medium ${hasQuestion ? "text-blue-600 dark:text-blue-400" : "text-slate-400 dark:text-slate-600"}`}>
                {hasQuestion ? "Yes" : "No"}
              </span>
            </div>

            <div className="flex items-center justify-between p-3 rounded-lg border border-slate-200 dark:border-slate-800/40">
              <span className="text-sm text-slate-700 dark:text-slate-300">Contains exclamation</span>
              <span className={`text-sm font-medium ${hasExclamation ? "text-amber-600 dark:text-amber-400" : "text-slate-400 dark:text-slate-600"}`}>
