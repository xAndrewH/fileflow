  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Regex Cheat Sheet</h1>
        ...
        <div className="relative mb-8">
          ...
          <input
            ...
            className="w-full bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl pl-11 pr-4 py-3 text-slate-800 dark:text-slate-200 text-sm focus:outline-none focus:border-blue-500/60 placeholder:text-slate-600 transition-colors"
          />
          {search && (
            <button onClick={() => setSearch("")} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 transition-colors">
        ...
        <div className="space-y-8">
          {filtered.map(cat => (
            <div key={cat.name}>
              <h2 className="text-slate-900 dark:text-white text-sm font-semibold mb-3 flex items-center gap-2">
                {cat.name}
                <span className="text-slate-400 dark:text-slate-600 text-xs font-normal">{cat.rows.length} patterns</span>
              </h2>
              <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {cat.rows.map((row, i) => (
                      <tr key={i} className="border-b border-slate-200 dark:border-slate-800/40 last:border-0 hover:bg-slate-100 dark:hover:bg-slate-800/30 transition-colors">
                        <td className="px-4 py-3 align-top">
                          <code className="font-mono text-blue-700 dark:text-blue-300 text-xs bg-slate-100 dark:bg-slate-800/70 px-1.5 py-0.5 rounded break-all">{row.pattern}</code>
                        </td>
                        <td className="px-4 py-3 text-slate-700 dark:text-slate-300 text-xs align-top leading-relaxed">{row.description}</td>
                        ...
                        <td className="px-4 py-3 align-top">
                          <button
                            ...
                            className="px-2 py-1 text-xs bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 rounded text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors whitespace-nowrap"
                          >
