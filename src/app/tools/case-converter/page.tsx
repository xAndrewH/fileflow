  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Case Converter</h1>
        ...
        <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl overflow-hidden">
          ...
          <textarea
            ...
            rows={10}
            className="w-full bg-transparent px-5 py-5 text-slate-800 dark:text-slate-200 text-sm leading-relaxed resize-none focus:outline-none placeholder:text-slate-600"
            spellCheck={false}
          />
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 dark:border-slate-800/60">
            <div className="flex items-center gap-1">
              ...
              {/* Download */}
              <button onClick={download} title="Download" className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
              ...
              {/* Clear */}
              <button onClick={() => setText("")} title="Clear" className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {/* Case buttons */}
          <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2 px-4 py-4 border-t border-slate-200 dark:border-slate-800/60 bg-slate-50 dark:bg-slate-900/40">
            {CASES.map(c => (
              <button
                key={c.id}
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${
                  activeCase === c.id
                    ? "border-white/30 text-slate-900 dark:text-white " + c.color
                    : "bg-slate-100 dark:bg-slate-800/60 border-slate-300 dark:border-slate-700/50 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800"
                }`}>
                <span className={`inline-flex items-center justify-center w-5 h-5 rounded text-[10px] font-bold text-slate-900 dark:text-white ${c.color}`}>
                  {c.abbr[0]}
                </span>
                {c.label}
              </button>
