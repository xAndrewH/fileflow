...
import type { SpreadsheetFormat } from "@/lib/xlsx-client";

const FORMATS: { id: SpreadsheetFormat; label: string }[] = [
  { id: "csv",  label: "CSV" },
  { id: "json", label: "JSON" },
  { id: "xlsx", label: "XLSX" },
];

const SHEET_EXT_RE = /\.(xlsx|xls|csv|json)$/i;
...
export default function XlsxConverterPage() {
  ...
  const addFiles = useCallback((files: File[]) => {
    const valid = files.filter((f) => SHEET_EXT_RE.test(f.name));
    if (valid.length === 0) { setError("Please select spreadsheet files (XLSX, XLS, CSV, or JSON)."); return; }
    setError("");
    setSheets((prev) => [
      ...prev,
  ...
  const remove = (id: string) => setSheets((prev) => {
  ...
  const clearAll = () => {
    sheets.forEach((i) => { if (i.resultUrl) URL.revokeObjectURL(i.resultUrl); });
    setSheets([]);
    setError("");
  };
  ...
  const convertAll = async () => {
    ...
    sheets.forEach((i) => { if (i.resultUrl) URL.revokeObjectURL(i.resultUrl); });
    setSheets((prev) => prev.map((i) => ({ ...i, status: "pending", resultUrl: undefined, resultSize: undefined, error: undefined })));

    const { convertSpreadsheetClient } = await import("@/lib/xlsx-client");
    for (const item of sheets) {
      try {
        const blob = await convertSpreadsheetClient(item.file, fmt.id);
        const url = URL.createObjectURL(blob);
        setSheets((prev) => prev.map((i) => i.id === item.id ? { ...i, status: "done", resultUrl: url, resultSize: blob.size } : i));
      } catch (e) {
        setSheets((prev) => prev.map((i) => i.id === item.id ? { ...i, status: "error", error: (e as Error).message } : i));
      }
    }
    setProcessing(false);
  };

  const downloadOne = (item: SheetFile) => {
  ...
  const downloadAll = async () => {
    const done = sheets.filter((i) => i.status === "done" && i.resultUrl);
  ...
  const doneCount = sheets.filter((i) => i.status === "done").length;
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">XLSX Converter</h1>
          <p className="text-slate-500 text-sm">Convert spreadsheets between XLSX, CSV, and JSON, in bulk, entirely in your browser.</p>
        </div>
        ...
        >
          ...
          <div className="text-center">
            <p className="font-medium text-sm">Drop spreadsheets here</p>
            <p className="text-xs opacity-60 mt-0.5">or click to browse — XLSX, XLS, CSV, JSON</p>
          </div>
          <input
            ref={inputRef}
            type="file"
        ...
        <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-4 space-y-4 mb-5">
          ...
          <p className="text-slate-500 text-xs">For multi-sheet workbooks, only the first sheet is used.</p>
        </div>
          ...
          <div className="flex gap-2">
            ...
            >
              {processing ? (
                ...
                `Convert ${sheets.length} file${sheets.length !== 1 ? "s" : ""} to ${fmt.label}`
              )}
            </button>
            {doneCount > 0 && (
        ...
        <RelatedTools current="/tools/xlsx-converter" />
      </div>
    </div>
  );
}
