...
import type { DocxOutputFormat } from "@/lib/docx-client";

const FORMATS: { id: DocxOutputFormat; label: string }[] = [
  { id: "html", label: "HTML" },
  { id: "text", label: "Text" },
];
...
export default function DocxConverterPage() {
  const [docs, setDocs]         = useState<DocxFile[]>([]);
  const [format, setFormat]     = useState<DocxOutputFormat>("html");
  ...
  const addFiles = useCallback((files: File[]) => {
    const valid = files.filter((f) => DOCX_EXT_RE.test(f.name));
    if (valid.length === 0) { setError("Please select .docx files."); return; }
    setError("");
    setDocs((prev) => [
      ...prev,
  ...
  const remove = (id: string) => setDocs((prev) => {
  ...
  const clearAll = () => {
    docs.forEach((i) => { if (i.resultUrl) URL.revokeObjectURL(i.resultUrl); });
    setDocs([]);
    setError("");
  };
  ...
  const convertAll = async () => {
    ...
    docs.forEach((i) => { if (i.resultUrl) URL.revokeObjectURL(i.resultUrl); });
    setDocs((prev) => prev.map((i) => ({ ...i, status: "pending", resultUrl: undefined, resultSize: undefined, error: undefined })));

    const { convertDocxClient } = await import("@/lib/docx-client");
    for (const item of docs) {
      try {
        const blob = await convertDocxClient(item.file, fmt.id);
        const url = URL.createObjectURL(blob);
        setDocs((prev) => prev.map((i) => i.id === item.id ? { ...i, status: "done", resultUrl: url, resultSize: blob.size } : i));
      } catch (e) {
        setDocs((prev) => prev.map((i) => i.id === item.id ? { ...i, status: "error", error: (e as Error).message } : i));
      }
    }
    setProcessing(false);
  };

  const downloadOne = (item: DocxFile) => {
  ...
  const downloadAll = async () => {
    const done = docs.filter((i) => i.status === "done" && i.resultUrl);
  ...
  const doneCount = docs.filter((i) => i.status === "done").length;
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">DOCX Converter</h1>
          <p className="text-slate-500 text-sm">Convert Word documents to HTML or plain text, in bulk, entirely in your browser.</p>
        </div>
        ...
        >
          ...
          <div className="text-center">
            <p className="font-medium text-sm">Drop Word documents here</p>
            <p className="text-xs opacity-60 mt-0.5">or click to browse — .docx files</p>
          </div>
          <input
            ref={inputRef}
            type="file"
          ...
          <div className="flex gap-2">
            ...
            >
              {processing ? (
                ...
                `Convert ${docs.length} file${docs.length !== 1 ? "s" : ""} to ${fmt.label}`
              )}
            </button>
            {doneCount > 0 && (
        ...
        <RelatedTools current="/tools/docx-converter" />
      </div>
    </div>
  );
}
