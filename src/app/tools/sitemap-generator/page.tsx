  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-slate-900 dark:text-white text-sm font-semibold">Fetch from site</h2>
      </div>
        ...
        {(["replace", "append"] as const).map(m => (
          <button
            ...
            className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors border ${
              mode === m ? "bg-slate-200 dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600" : "text-slate-500 border-transparent hover:text-slate-800 dark:hover:text-slate-300"
            }`}
          >
      ...
      {status && (
        <div className={`rounded-xl px-3 py-2.5 text-xs space-y-1 ${status.ok ? "bg-green-500/10 border border-green-500/20" : "bg-red-500/10 border border-red-500/20"}`}>
          <p className={`font-medium ${status.ok ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-5xl mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Sitemap Generator</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5 space-y-3">
              <div className="flex items-center justify-between">
                <h2 className="text-slate-900 dark:text-white text-sm font-semibold">URLs</h2>
              ...
              <textarea
                ...
                className="w-full bg-slate-100 dark:bg-slate-800/60 border border-slate-300 dark:border-slate-700/50 rounded-lg px-3 py-2 text-slate-800 dark:text-slate-200 text-sm font-mono focus:outline-none focus:border-blue-500/60 focus:ring-1 focus:ring-blue-500/30 transition-colors resize-none"
              />
              {invalidUrls.length > 0 && (
                <div className="space-y-1">
                  <p className="text-xs font-medium text-red-600 dark:text-red-400">Invalid URLs (excluded):</p>
            ...
            <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5 space-y-4">
              <h2 className="text-slate-900 dark:text-white text-sm font-semibold">Global Settings</h2>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">Change Freq</label>
                  <select value={globalFreq} onChange={e => setGlobalFreq(e.target.value as Changefreq)}
                    className="w-full bg-slate-100 dark:bg-slate-800/60 border border-slate-300 dark:border-slate-700/50 rounded-lg px-3 py-2 text-slate-800 dark:text-slate-200 text-sm focus:outline-none focus:border-blue-500/60 [color-scheme:dark]">
                    {CHANGEFREQ_OPTIONS.map(f => <option key={f} value={f}>{f}</option>)}
                  </select>
                </div>
                <div className="space-y-1.5">
                  <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">Priority</label>
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">Last Modified</label>
            ...
            {validUrls.length > 0 && (
              <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5 space-y-3">
                <h2 className="text-slate-900 dark:text-white text-sm font-semibold">Per-URL Overrides</h2>
                    ...
                    return (
                      <button key={url}
                        ...
                        className={`w-full text-left px-3 py-2 rounded-lg text-xs font-mono transition-colors flex items-center gap-2 ${
                          selectedUrl === url
                            ? "bg-blue-600/20 border border-blue-500/40 text-blue-700 dark:text-blue-300"
                            : "bg-slate-100 dark:bg-slate-800/40 border border-slate-300 dark:border-slate-700/40 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800/70"
                        }`}>
                        <span className="flex-1 truncate">{url}</span>
                        {hasOverride && <span className="shrink-0 text-[9px] bg-blue-600/30 text-blue-600 dark:text-blue-400 px-1.5 py-0.5 rounded-full border border-blue-500/30">custom</span>}
                      </button>
                    );
                  })}
                </div>
                ...
                {selectedUrl && (
                  <div className="border-t border-slate-200 dark:border-slate-800/60 pt-3 space-y-3">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-medium text-slate-700 dark:text-slate-300 truncate max-w-[200px]">{selectedUrl}</p>
                      {overrides[selectedUrl] && Object.values(overrides[selectedUrl]).some(Boolean) && (
                        <button onClick={() => setOverrides(prev => { const n = {...prev}; delete n[selectedUrl]; return n; })}
                          className="text-xs text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 transition-colors">Clear overrides</button>
                      )}
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1.5">
                        <label className="block text-xs font-medium text-slate-500 dark:text-slate-400">Change Freq</label>
                    ...
                    <div className="space-y-1.5">
                      <label className="block text-xs font-medium text-slate-500 dark:text-slate-400">Last Modified</label>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-slate-900 dark:text-white text-sm font-semibold">sitemap.xml <span className="text-slate-500 font-normal">({validUrls.length} URLs)</span></h2>
              <div className="flex gap-2">
                <button onClick={copy}
                  className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 rounded-lg text-xs font-medium text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors">
                  {copied ? "Copied!" : "Copy XML"}
                </button>
                <button onClick={downloadUrls} disabled={validUrls.length === 0}
                  className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-40 border border-slate-300 dark:border-slate-700 rounded-lg text-xs font-medium text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors">
                  URLs (.txt)
                </button>
            ...
            <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-4 overflow-auto" style={{ maxHeight: "700px" }}>
              <pre className="text-slate-700 dark:text-slate-300 text-xs font-mono whitespace-pre leading-relaxed">{xml}</pre>
            </div>
          </div>
        </div>
      </div>
