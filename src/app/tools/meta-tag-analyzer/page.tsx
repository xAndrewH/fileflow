    ...
    <div className="flex flex-col items-center gap-2">
      <div className="relative w-24 h-24">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-2xl font-bold text-slate-900 dark:text-white leading-none">{score}</span>
        </div>
      </div>
      <div>
        <p className="text-slate-500 dark:text-slate-400 text-xs text-center">Meta Tag Score</p>
...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <button onClick={copy} className="p-1 rounded text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 transition-colors">
...
function BasicRow({ label, value, lengthBadge }: { label: string; value: string; lengthBadge?: boolean }) {
  ...
  return (
    <div className="flex items-start gap-3 py-2.5 border-b border-slate-200 dark:border-slate-800/60 last:border-0">
      <span className="text-slate-500 text-xs w-28 flex-shrink-0 pt-0.5">{label}</span>
      <div className="flex-1 min-w-0">
        {value ? (
          <div className="flex items-start gap-2">
            <span className="text-slate-800 dark:text-slate-200 text-sm font-mono break-all">{display}</span>
            {isTrunc && (
              <button onClick={() => setExpanded(e => !e)} className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 flex-shrink-0 mt-0.5">
                {expanded ? "less" : "more"}
              </button>
            )}
          </div>
        ) : (
          <span className="text-slate-400 dark:text-slate-600 text-sm italic">|</span>
        )}
        {lengthBadge && value && (
          <span className={`mt-1 inline-block text-xs px-1.5 py-0.5 rounded font-mono ${value.length > 160 || (label === "Title" && value.length > 60) ? "bg-orange-500/20 text-orange-600 dark:text-orange-400" : "bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400"}`}>
            {value.length} chars
          </span>
        )}
      </div>
...
function TagRow({ name, content }: { name: string; content: string }) {
  ...
  return (
    <tr className="border-b border-slate-200 dark:border-slate-800/40 last:border-0">
      <td className="py-2 pr-4 font-mono text-xs text-blue-600 dark:text-blue-400 whitespace-nowrap align-top">{name}</td>
      <td className="py-2 text-sm text-slate-700 dark:text-slate-300 break-all">
        <div className="flex items-start gap-2">
          <span>{display}</span>
          {isTrunc && (
            <button onClick={() => setExpanded(e => !e)} className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 flex-shrink-0">
              {expanded ? "less" : "more"}
            </button>
          )}
        </div>
        {isUrl && name.toLowerCase().includes("image") && (
          <img src={content} alt="" className="mt-1.5 h-16 w-auto rounded border border-slate-300 dark:border-slate-700/60 object-cover" onError={e => { (e.currentTarget as HTMLImageElement).style.display = "none"; }} />
        )}
      </td>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Meta Tag Analyzer</h1>
          <p className="text-slate-500 text-sm">Audit any URL's meta tags, Open Graph, and Twitter Card data.</p>
        </div>

        <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-5 flex gap-3">
          <input
            type="url"
            value={url}
        ...
        {error && (
          <div className="mt-4 bg-red-500/10 border border-red-500/20 rounded-lg p-4 text-red-600 dark:text-red-400 text-sm">
            {error}
          </div>
        )}
        ...
        {result && !loading && (
          <div className="mt-6 space-y-5">
            <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-5 flex flex-col sm:flex-row gap-6 items-center sm:items-start">
              ...
              {result.issues.length === 0 && (
                <div className="flex-1 flex items-center">
                  <p className="text-green-600 dark:text-green-400 text-sm">No issues found. All essential meta tags are present.</p>
                </div>
              )}
            </div>

            <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-5">
              <h2 className="text-slate-900 dark:text-white text-sm font-semibold mb-3">Basic SEO</h2>
            ...
            {result.og.length > 0 && (
              <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-5">
                <h2 className="text-slate-900 dark:text-white text-sm font-semibold mb-3">Open Graph</h2>
            ...
            {result.twitter.length > 0 && (
              <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-5">
                <h2 className="text-slate-900 dark:text-white text-sm font-semibold mb-3">Twitter Card</h2>
                <table className="w-full">
                  <tbody>
            ...
            {result.other.length > 0 && (
              <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-5">
                ...
                >
                  <h2 className="text-slate-900 dark:text-white text-sm font-semibold">Other Tags</h2>
