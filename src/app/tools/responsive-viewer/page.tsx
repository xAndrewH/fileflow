...
function DeviceCard({ device, shot, loading }: { device: Device; shot?: Shot; loading: boolean }) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-3 py-2 border-b border-slate-200 dark:border-slate-800/60 bg-slate-50 dark:bg-slate-900/80">
        <span className="text-slate-700 dark:text-slate-300 text-xs font-medium truncate">{device.name}</span>
        <span className="text-slate-400 dark:text-slate-600 text-[11px] tabular-nums shrink-0 ml-2">
          {device.width}×{device.height}
        </span>
      </div>
      <div className="flex items-center justify-center bg-white dark:bg-slate-950/60 p-3">
        <div
        ...
        >
          {loading && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-slate-100 animate-pulse">
              ...
              <span className="text-slate-500 dark:text-slate-400 text-[11px]">Capturing…</span>
            </div>
          )}
          {!loading && shot?.error && (
          ...
          {!loading && !shot && (
            <span className="m-auto text-slate-500 dark:text-slate-400 text-[11px]">Not captured yet</span>
          )}
        </div>
      </div>
      {!loading && shot?.image && (
        <p className="px-3 -mt-1 pb-1 text-slate-400 dark:text-slate-600 text-[10px]">Scroll inside the frame to see the full page</p>
      )}
      {shot?.image && (
        <div className="px-3 pb-3 -mt-1 flex justify-end">
          <a
            ...
            className="inline-flex items-center gap-1 text-[11px] text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 transition-colors"
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1 flex items-center gap-2.5">
        ...
        <div className="flex gap-2 mb-4">
          ...
          {url && (
            <a
              ...
              className="px-3 py-2.5 rounded-lg text-sm font-medium bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 transition-colors border border-slate-300 dark:border-slate-700/60"
        ...
        <div className="mb-8 space-y-3">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                  selected.includes(d.id)
                    ...
                    : "bg-slate-100 dark:bg-slate-800/60 border-slate-300 dark:border-slate-700/50 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 hover:border-slate-400 dark:hover:border-slate-600"
                }`}
                type="button"
              >
              ...
              <span
                ...
                className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                  selected.includes(d.id)
                    ...
                    : "bg-slate-100 dark:bg-slate-800/60 border-slate-300 dark:border-slate-700/50 text-slate-500 dark:text-slate-400"
                }`}
              >
          ...
          <div className="flex flex-wrap items-center gap-2">
            <input
              ...
              placeholder="Width"
              className="w-24 bg-slate-100 dark:bg-slate-800/60 border border-slate-300 dark:border-slate-700/50 rounded-lg px-3 py-1.5 text-slate-900 dark:text-white text-xs focus:outline-none focus:border-blue-500/60 placeholder:text-slate-600"
            />
            ...
            <button
              ...
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed text-slate-700 dark:text-slate-300 transition-colors border border-slate-300 dark:border-slate-700/60"
              type="button"
            >
        ...
        {!url && !loading && (
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-8 text-center">
            <Camera className="w-8 h-8 text-slate-300 dark:text-slate-700 mx-auto mb-3" />
        ...
        {url && !loading && activeDevices.length === 0 && (
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-8 text-center">
            <p className="text-slate-500 dark:text-slate-400 text-sm">Select at least one device above, then capture again.</p>
          </div>
        )}

        <p className="text-slate-400 dark:text-slate-600 text-xs mt-6">
