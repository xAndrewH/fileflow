  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Timestamp Converter</h1>
        <p className="text-slate-500 text-sm mb-8">Convert Unix timestamps to human-readable dates and back.</p>

        <div className="space-y-5">
          {/* Live clock */}
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-4 flex items-center justify-between gap-4">
            <div>
              <p className="text-slate-500 dark:text-slate-400 text-xs mb-1">Current Unix timestamp</p>
              <p className="text-slate-900 dark:text-white font-mono text-xl">{Math.floor(now.getTime() / 1000)}</p>
            ...
            <div className="flex flex-col gap-2">
              ...
              <label className="flex items-center gap-1.5 cursor-pointer">
                ...
                <span className="text-slate-500 dark:text-slate-400 text-xs">Live</span>
              </label>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-slate-500 dark:text-slate-400 text-xs">Enter a Unix timestamp or date string</label>
            ...
            <input
              ...
              spellCheck={false}
              className={`w-full bg-slate-50 dark:bg-slate-900/60 border rounded-xl px-4 py-3 text-slate-900 dark:text-white font-mono text-sm focus:outline-none transition-colors ${error ? "border-red-500/50" : "border-slate-200 dark:border-slate-800/60 focus:border-blue-500/50"}`}
            />
          ...
          {display && (
            <div className="space-y-2">
              {FORMATS.map(({ label, fn }) => {
                ...
                return (
                  <div key={label} className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl px-4 py-3 flex items-center gap-3">
                    <span className="text-slate-500 text-xs w-24 shrink-0">{label}</span>
                    <code className="flex-1 text-slate-900 dark:text-white text-sm font-mono truncate">{val}</code>
