...
function SkeletonRow() {
  return <div className="h-4 bg-slate-100 dark:bg-slate-800 rounded animate-pulse" />;
}
...
export default function SslCheckerPage() {
  ...
  const statusBadge = result
    ? result.isExpired
      ? { label: "Expired", classes: "bg-red-500/20 text-red-600 dark:text-red-400 border-red-500/40" }
      : result.isExpiringSoon
        ? { label: "Expiring Soon", classes: "bg-yellow-500/20 text-yellow-600 dark:text-yellow-400 border-yellow-500/40" }
        : { label: "Valid", classes: "bg-green-500/20 text-green-600 dark:text-green-400 border-green-500/40" }
    : null;
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">SSL Certificate Checker</h1>
        ...
        <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5">
          <div className="flex gap-3">
            <div className="flex-1 space-y-1.5">
              <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">Domain</label>
              <input
                value={input}
                onChange={e => setInput(e.target.value)}
                onKeyDown={handleKey}
                placeholder="example.com or https://example.com"
                className="w-full bg-slate-100 dark:bg-slate-800/60 border border-slate-300 dark:border-slate-700/50 rounded-lg px-3 py-2 text-slate-800 dark:text-slate-200 text-sm focus:outline-none focus:border-blue-500/60 focus:ring-1 focus:ring-blue-500/30 transition-colors"
              />
            </div>
        ...
        {/* Loading skeleton */}
        {loading && (
          <div className="mt-5 bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5 space-y-4">
            ...
            <div className="space-y-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="flex gap-4">
                  <div className="w-24 shrink-0 h-4 bg-slate-100 dark:bg-slate-800 rounded animate-pulse" />
        ...
        {error && !loading && (
          <div className="mt-5 bg-red-500/10 border border-red-500/30 rounded-2xl p-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-red-600 dark:text-red-400 text-sm font-medium">Could not check {checkedHost}</p>
        ...
        {result && !loading && statusBadge && (
          <div className="mt-5 bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5 space-y-5">
            {/* Header */}
            <div className="flex items-center gap-3 flex-wrap">
              <span className={`px-3 py-1 text-xs font-semibold rounded-full border ${statusBadge.classes}`}>
                {statusBadge.label}
              </span>
              <span className="text-slate-700 dark:text-slate-300 text-sm font-medium">{checkedHost}</span>
            </div>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={label} className="flex gap-4">
                  ...
                  <span className="text-slate-800 dark:text-slate-200 text-sm">{value}</span>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-sm font-semibold ${result.isExpired ? "text-red-600 dark:text-red-400" : result.isExpiringSoon ? "text-yellow-600 dark:text-yellow-400" : "text-green-600 dark:text-green-400"}`}>
                  {result.isExpired
            ...
            {result.sans.length > 0 && (
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {result.sans.map(san => (
                    <span key={san} className="text-xs bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700/60 text-slate-700 dark:text-slate-300 px-2 py-0.5 rounded-md font-mono">
                      {san}
                    </span>
                  ))}
                </div>
            ...
            {result.fingerprint256 && (
              <div className="space-y-1.5">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-xs font-mono break-all bg-slate-100 dark:bg-slate-800/60 border border-slate-300 dark:border-slate-700/40 rounded-lg px-3 py-2 leading-relaxed">
                  {result.fingerprint256}
                </p>
              </div>
            )}
