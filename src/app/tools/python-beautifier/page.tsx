  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Python Beautifier</h1>
        ...
        <div className="flex justify-end gap-2 mb-3">
          ...
          <button onClick={() => { setInput(EXAMPLE.replace(/\\n/g, "\n")); setOutput(""); }} className="text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 text-xs px-2 py-1.5 transition-colors">
            Load example
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              {input && <button onClick={() => { setInput(""); setOutput(""); }} className="text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 text-xs transition-colors">Clear</button>}
            </div>
            <textarea
              value={input}
          ...
          <div className="space-y-2">
            ...
            <pre className="w-full h-[520px] bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl px-4 py-4 text-slate-800 dark:text-slate-200 text-xs font-mono leading-relaxed overflow-auto whitespace-pre">
              {output || <span className="text-slate-400 dark:text-slate-600">Formatted Python will appear here…</span>}
            </pre>
          </div>
        </div>
