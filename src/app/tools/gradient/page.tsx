  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">CSS Gradient Builder</h1>
        <p className="text-slate-500 text-sm mb-8">Build linear and radial gradients visually and copy the CSS.</p>

        <div className="space-y-5">
          {/* Preview */}
          <div className="h-40 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl transition-all duration-300"
          ...
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-4 space-y-4">
            <div className="flex items-center gap-4">
              <span className="text-slate-900 dark:text-white text-sm font-medium w-16">Type</span>
              <div className="flex gap-2">
                {(["linear", "radial"] as const).map(t => (
                  <button key={t} onClick={() => setType(t)}
                    className={`px-4 py-1.5 rounded-lg text-sm capitalize transition-colors ${type === t ? "bg-blue-600 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}>
                    {t}
                  </button>
                ))}
              </div>
            ...
            {type === "linear" && (
              <div>
                <div className="flex justify-between mb-2">
                  <span className="text-slate-900 dark:text-white text-sm font-medium">Angle</span>
                  <span className="text-blue-600 dark:text-blue-400 font-mono text-sm">{angle}°</span>
                </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-slate-900 dark:text-white text-sm font-medium">Color stops</p>
            ...
            {stops.map((stop, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <button onClick={() => removeStop(i)}
                  className="text-slate-400 dark:text-slate-600 hover:text-red-600 dark:hover:text-red-400 transition-colors text-lg leading-none"
              ...
              {PRESETS.map(p => {
                ...
                return (
                  <button key={p.label} onClick={() => setStops(p.stops)}
                    className="group rounded-xl overflow-hidden border border-slate-300 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-500 transition-colors">
                    <div className="h-10" style={{ background: previewCss }} />
                    <div className="px-2 py-1.5 bg-slate-100 dark:bg-slate-800/60">
                      <p className="text-slate-500 dark:text-slate-400 text-xs group-hover:text-white transition-colors">{p.label}</p>
                    </div>
                  </button>
                );
              })}
