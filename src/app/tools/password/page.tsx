  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Password Generator</h1>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-1 bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-1 w-fit">
            {(["password", "passphrase"] as Mode[]).map(m => (
              <button key={m} onClick={() => setMode(m)}
                className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-colors capitalize ${mode === m ? "bg-blue-600 text-white" : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}>
                {m}
              </button>
            ))}
          </div>
          ...
          {mode === "password" ? (
            <>
              <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <label className="text-slate-900 dark:text-white text-sm font-medium">Length</label>
                  <div className="flex items-center gap-2">
                    <button onClick={() => setLength(l => Math.max(4, l - 1))} className="w-6 h-6 flex items-center justify-center bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-md text-sm transition-colors">−</button>
                    <span className="text-blue-600 dark:text-blue-400 font-mono text-sm font-bold w-8 text-center">{length}</span>
                    <button onClick={() => setLength(l => Math.min(128, l + 1))} className="w-6 h-6 flex items-center justify-center bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-md text-sm transition-colors">+</button>
                  </div>
                </div>
                  ...
                  <div className="flex gap-3">
                    {[8,12,16,24,32].map(n => (
                      <button key={n} onClick={() => setLength(n)}
                        className={`text-[10px] transition-colors ${length === n ? "text-blue-600 dark:text-blue-400" : "hover:text-slate-800 dark:hover:text-slate-300"}`}>{n}</button>
                    ))}
                  </div>
                  <span>128</span>
                </div>
              ...
              <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-4">
                <p className="text-slate-900 dark:text-white text-sm font-medium mb-3">Character types</p>
                    ...
                    <button key={key} onClick={() => toggle(key)}
                      className={`flex items-center gap-2 px-3 py-2.5 rounded-lg border text-sm transition-colors ${
                        opts[key as keyof typeof opts]
                          ...
                          : "bg-slate-100 dark:bg-slate-800/40 border-slate-300 dark:border-slate-700/50 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
                      }`}>
                      <span className={`w-4 h-4 rounded flex items-center justify-center shrink-0 ${opts[key as keyof typeof opts] ? "bg-blue-500" : "bg-slate-200 dark:bg-slate-700"}`}>
                        {opts[key as keyof typeof opts] && (
                          <svg className="w-3 h-3 text-slate-900 dark:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                        )}
                      </span>
                      <span>{label}</span>
                ...
                <div className="flex items-center gap-3 cursor-pointer mt-3 pt-3 border-t border-slate-200 dark:border-slate-800/60" onClick={() => toggle("excludeSimilar")}>
                  <div className={`w-8 h-4 rounded-full transition-colors relative shrink-0 ${opts.excludeSimilar ? "bg-blue-500" : "bg-slate-200 dark:bg-slate-700"}`}>
                    <div className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-all ${opts.excludeSimilar ? "left-4" : "left-0.5"}`} />
                  </div>
                  <span className="text-slate-700 dark:text-slate-300 text-sm select-none">Exclude similar characters</span>
            ...
            <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-4 space-y-4">
              <div>
                <div className="flex items-center justify-between mb-3">
                  <label className="text-slate-900 dark:text-white text-sm font-medium">Word count</label>
                  <div className="flex items-center gap-2">
                    <button onClick={() => setWordCount(w => Math.max(2, w - 1))} className="w-6 h-6 flex items-center justify-center bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-md text-sm transition-colors">−</button>
                    <span className="text-blue-600 dark:text-blue-400 font-mono text-sm font-bold w-6 text-center">{wordCount}</span>
                    <button onClick={() => setWordCount(w => Math.min(8, w + 1))} className="w-6 h-6 flex items-center justify-center bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-md text-sm transition-colors">+</button>
                  </div>
                </div>
                <div className="flex gap-1">
                  {[2,3,4,5,6,8].map(n => (
                    <button key={n} onClick={() => setWordCount(n)}
                      className={`px-2.5 py-1 rounded-md text-xs transition-colors ${wordCount === n ? "bg-blue-600 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}>{n}</button>
                  ))}
                </div>
              </div>
              <div>
                <p className="text-slate-900 dark:text-white text-sm font-medium mb-2">Separator</p>
                <div className="grid grid-cols-1 gap-1.5">
                  {SEPARATORS.map(s => (
                    <button key={s.value} onClick={() => setSeparator(s.value)}
                      ...
                      <span className={`w-3.5 h-3.5 rounded-full border flex-shrink-0 flex items-center justify-center ${separator === s.value ? "border-blue-400 bg-blue-500" : "border-slate-300 dark:border-slate-600"}`}>
              ...
              <div className="flex items-center gap-3 cursor-pointer" onClick={() => setCapitalize(c => !c)}>
                <div className={`w-8 h-4 rounded-full transition-colors relative ${capitalize ? "bg-blue-500" : "bg-slate-200 dark:bg-slate-700"}`}>
                  <div className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-all ${capitalize ? "left-4" : "left-0.5"}`} />
                </div>
                <span className="text-slate-700 dark:text-slate-300 text-sm select-none">Capitalize each word</span>
              </div>
            </div>
          )}
          ...
          <div className="flex gap-3 items-stretch">
            <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl px-4 py-2">
              <span className="text-slate-500 dark:text-slate-400 text-sm">Generate</span>
              <select value={count} onChange={e => setCount(+e.target.value)} className="bg-transparent text-slate-900 dark:text-white text-sm focus:outline-none">
            ...
            <>
              <div className="space-y-2">
                {passwords.map((pw, i) => {
                  ...
                  return (
                    <div key={i}
                      className={`group bg-slate-50 dark:bg-slate-900/60 border rounded-xl p-3 transition-colors cursor-pointer ${isCopied ? "border-green-500/40 bg-green-500/5" : "border-slate-200 dark:border-slate-800/60 hover:border-slate-400 dark:hover:border-slate-700"}`}
                      onClick={() => copy(pw)} title="Click to copy">
                      <div className="flex items-center gap-3">
                        <code className="flex-1 text-slate-900 dark:text-white text-sm font-mono break-all leading-relaxed">{pw}</code>
                        <div className={`shrink-0 w-8 h-8 flex items-center justify-center rounded-lg border transition-colors ${isCopied ? "bg-green-600 border-green-500 text-white" : "bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-slate-500 dark:text-slate-400 group-hover:text-white"}`}>
                          {isCopied ? (
