...
function Badge({ pass, label, requirement }: { pass: boolean; label: string; requirement: string }) {
  return (
    <div className={`flex flex-col items-center gap-1 rounded-xl border px-3 py-3 ${pass ? "bg-green-500/15 border-green-500/30" : "bg-red-500/15 border-red-500/30"}`}>
      <span className={`text-xs font-semibold ${pass ? "text-green-700 dark:text-green-300" : "text-red-700 dark:text-red-300"}`}>{label}</span>
      <span className={`text-xs font-bold ${pass ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>{pass ? "Pass ✓" : "Fail ✗"}</span>
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative w-12 h-12 flex-shrink-0 rounded-xl border border-slate-300 dark:border-slate-700 overflow-hidden shadow-sm">
        ...
        <div className="flex-1 min-w-0">
          <input
            ...
            placeholder="#000000"
            className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2.5 text-slate-900 dark:text-white font-mono text-sm focus:outline-none focus:border-blue-500/60 transition-colors uppercase tracking-widest"
            maxLength={7}
            spellCheck={false}
          />
        </div>
...
export default function ColorContrastPage() {
  ...
  const ratioColor = ratio >= 7 ? "text-green-600 dark:text-green-400" : ratio >= 4.5 ? "text-blue-600 dark:text-blue-400" : ratio >= 3 ? "text-yellow-600 dark:text-yellow-400" : "text-red-600 dark:text-red-400";
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-lg mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Color Contrast Checker</h1>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5 space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={swap}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white text-xs transition-colors"
          ...
          {/* ── Preview ── */}
          <div className="rounded-2xl p-5 space-y-3 border border-slate-300 dark:border-slate-700/60" style={{ background: validBg }}>
          ...
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5">
            <p className="text-slate-900 dark:text-white text-sm font-medium mb-3">WCAG Results</p>
            ...
            <p className="text-slate-400 dark:text-slate-600 text-[11px] mt-3">Large text: 18pt+ regular or 14pt+ bold. UI Components use AA Large threshold.</p>
          </div>

          {/* ── Suggested pairs ── */}
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5">
            <p className="text-slate-900 dark:text-white text-sm font-medium mb-3">Suggested pairs</p>
              ...
              {SUGGESTED_PAIRS.map((pair) => {
                ...
                return (
                  <button
                    ...
                    className="flex items-center gap-3 rounded-xl border border-slate-300 dark:border-slate-700/60 bg-slate-100 dark:bg-slate-800/40 hover:bg-slate-100 dark:hover:bg-slate-800 hover:border-slate-400 dark:hover:border-slate-600 transition-colors px-3 py-2.5 text-left"
                  >
                    <div
                    ...
                    <div className="min-w-0">
                      <p className="text-slate-700 dark:text-slate-300 text-xs font-medium truncate">{pair.label}</p>
                      <p className={`text-[11px] font-mono ${passes ? "text-green-600 dark:text-green-400" : "text-yellow-600 dark:text-yellow-400"}`}>{r.toFixed(1)}:1</p>
                    </div>
                  </button>
                );
              })}
