...
<body>${renderedHtml}</body>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 flex flex-col">
      {/* Header */}
      <div className="border-b border-slate-200 dark:border-slate-800/60 px-4 py-3 flex items-center gap-4 flex-wrap">
        <Link href="/tools" className="inline-flex items-center gap-1.5 text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 text-sm transition-colors">
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
          Tools
        </Link>
        <span className="text-slate-900 dark:text-white font-semibold">Markdown Editor</span>
        <span className="text-slate-400 dark:text-slate-600 text-xs ml-1">{wordCount} words · {charCount} chars</span>
        <div className="flex gap-1 ml-auto flex-wrap">
          {([["split", "Split"], ["editor", "Editor"], ["preview", "Preview"]] as const).map(([v, label]) => (
            <button key={v} onClick={() => setView(v)}
              className={`px-3 py-1 rounded-lg text-xs transition-colors ${view === v ? "bg-blue-600 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}>
              {label}
            </button>
          ))}
          <button onClick={() => setMarkdown(SAMPLE_MARKDOWN)}
            className="ml-2 px-3 py-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded-lg transition-colors">
            Try an example
          </button>
          <CopyButton text={() => renderedHtml} label="Copy HTML" className="ml-1" />
          <div className="relative ml-1">
            <button onClick={() => setShowExport(o => !o)}
              className="px-3 py-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded-lg transition-colors flex items-center gap-1">
              Export
            ...
            {showExport && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1 z-20 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700/60 rounded-xl shadow-xl overflow-hidden w-40">
                  <button onClick={exportHtml}
                    className="w-full px-4 py-2.5 text-left text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 text-xs transition-colors">
                    Export as HTML
                  </button>
                  <button onClick={exportPdf}
                    className="w-full px-4 py-2.5 text-left text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 text-xs transition-colors border-t border-slate-200 dark:border-slate-800/60">
                    Export as PDF
                  </button>
                </div>
              </>
      ...
      {/* Panes */}
      <div className={`flex-1 grid ${view === "split" ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1"} divide-y sm:divide-y-0 divide-x-0 sm:divide-x divide-slate-200 dark:divide-slate-800/60 min-h-0`}
        style={{ height: "calc(100vh - 57px)" }}>
        {(view === "split" || view === "editor") && (
          <div className="flex flex-col">
            <div className="px-4 py-2 border-b border-slate-200 dark:border-slate-800/40 flex items-center gap-2">
            ...
            <textarea
              ...
              spellCheck={false}
              className="flex-1 bg-transparent p-4 text-slate-900 dark:text-white text-sm font-mono resize-none focus:outline-none leading-relaxed"
            />
          </div>
        )}
        {(view === "split" || view === "preview") && (
          <div className="flex flex-col overflow-hidden">
            <div className="px-4 py-2 border-b border-slate-200 dark:border-slate-800/40">
