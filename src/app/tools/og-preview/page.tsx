...
function SmartImage({
  ...
  if (!src || failed) {
    return (
      <div
        className={`${className} ${rounded} bg-slate-200 flex flex-col items-center justify-center text-slate-500 dark:text-slate-400 gap-1`}
      >
  ...
  return (
    ...
    <img
      src={src}
      alt={alt}
      className={`${className} ${rounded} object-cover bg-slate-100 dark:bg-slate-800`}
...
function CopyButton({ value }: { value: string }) {
  ...
  return (
    <button
      onClick={copy}
      className="shrink-0 text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 transition-colors"
      title="Copy value"
      type="button"
    >
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        {loading && (
          <div className="space-y-6 animate-pulse">
            {[0, 1, 2].map((i) => (
              <div key={i} className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5">
                <div className="h-3 w-24 bg-slate-100 dark:bg-slate-800 rounded mb-4" />
                <div className="aspect-[1.91/1] w-full bg-slate-100 dark:bg-slate-800 rounded-xl mb-3" />
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-6">
              ...
              <div>
                <p className="text-slate-500 text-xs font-medium mb-2 uppercase tracking-wide">
                  X / Twitter
                  <span className="ml-2 normal-case text-slate-400 dark:text-slate-600">
                ...
                <div className="max-w-[500px]">
                  <div className="relative">
                    <SmartImage
                      src={rImage}
                      alt="twitter preview"
                      className="w-full aspect-[1.91/1] border border-slate-300 dark:border-slate-700"
                      rounded="rounded-2xl"
                    />
                    {domain && rImage && (
            ...
            <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="text-slate-900 dark:text-white text-sm font-semibold">Checks</h2>
              </div>
                  ...
                  {data.warnings.map((w, i) => {
                    ...
                    return (
                      <li key={i} className="flex items-start gap-2.5 text-sm">
                        ...
                        />
                        <span className={critical ? "text-red-700 dark:text-red-300" : "text-slate-700 dark:text-slate-300"}>{w}</span>
                      </li>
                    );
                  })}
                </ul>
            ...
            <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-slate-900 dark:text-white text-sm font-semibold">What platforms will actually use</h2>
              </div>
              <dl className="space-y-3 text-sm">
                <div>
                  <dt className="text-slate-500 text-xs mb-0.5">Title</dt>
                  <dd className="text-slate-800 dark:text-slate-200">{rTitle || <span className="text-slate-400 dark:text-slate-600">|</span>}</dd>
                </div>
                <div>
                  <dt className="text-slate-500 text-xs mb-0.5">Description</dt>
                  <dd className="text-slate-800 dark:text-slate-200">{rDesc || <span className="text-slate-400 dark:text-slate-600">|</span>}</dd>
                </div>
                <div>
                  <dt className="text-slate-500 text-xs mb-0.5">Image</dt>
                  <dd className="text-slate-800 dark:text-slate-200 break-all flex items-start gap-1.5">
                    <Link2 className="w-3.5 h-3.5 mt-0.5 text-slate-500 shrink-0" />
                    {rImage || <span className="text-slate-400 dark:text-slate-600">none</span>}
                  </dd>
                </div>
                <div>
                  <dt className="text-slate-500 text-xs mb-0.5">Domain</dt>
                  <dd className="text-slate-800 dark:text-slate-200">{domain || <span className="text-slate-400 dark:text-slate-600">|</span>}</dd>
                </div>
              </dl>
            </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-2xl overflow-hidden">
              ...
              >
                ...
                <ChevronDown
                  className={`w-4 h-4 text-slate-500 dark:text-slate-400 transition-transform ${rawOpen ? "rotate-180" : ""}`}
                />
              </button>
              {rawOpen && (
                    ...
                    <div className="divide-y divide-slate-200 dark:divide-slate-800/60">
                      {rawTags.map((t, i) => (
                        <div key={i} className="flex items-start gap-3 py-2 text-xs font-mono">
                          <span className="text-blue-600 dark:text-blue-400 w-40 shrink-0">{t.property}</span>
                          <span className="text-slate-700 dark:text-slate-300 flex-1 break-all">{t.value}</span>
