  ...
  return (
    <div>
      ...
      <div className="relative">
        {prefix && (
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 dark:text-slate-400 text-sm pointer-events-none">{prefix}</span>
        )}
        <input
          type="number"
          value={value}
          ...
          placeholder="0"
          className={`w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700/60 rounded-lg ${prefix ? "pl-7" : "px-3"} ${suffix ? "pr-10" : "pr-3"} py-2.5 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:border-blue-500/60 transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none`}
        />
        {suffix && (
          <span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 dark:text-slate-400 text-sm pointer-events-none">{suffix}</span>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-lg mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">CPM / CPC / CTR Calculator</h1>
        <p className="text-slate-500 text-sm mb-8">
          Calculate key ad metrics. Choose two values to find the third.
        </p>

        <div className="mb-6">
          <label className="block text-sm text-slate-500 dark:text-slate-400 mb-1.5">Calculate</label>
          <select
            value={mode}
        ...
        <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-5 mt-8">
          <p className="text-slate-900 dark:text-white text-sm font-semibold mb-3">Quick Reference</p>
          <div className="flex flex-col gap-3">
            {quickRef.map((item) => (
              <div key={item.term} className="flex items-start gap-3">
                <span className="text-blue-600 dark:text-blue-400 text-xs font-bold font-mono w-12 shrink-0 pt-0.5">{item.term}</span>
                <div>
                  <p className="text-slate-700 dark:text-slate-300 text-sm">{item.def}</p>
                  <p className="text-slate-400 dark:text-slate-600 text-xs font-mono">{item.formula}</p>
                </div>
              </div>
            ))}
          </div>
