  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Cron Expression Builder</h1>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-4">
            <div className="flex items-center gap-3">
              ...
              />
              <button onClick={copy}
                className="px-4 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-sm rounded-lg transition-colors shrink-0">
                {copied ? "Copied!" : "Copy"}
              </button>
            </div>
            {isValid ? (
              <p className="text-blue-600 dark:text-blue-400 text-sm mt-2">{description}</p>
            ) : (
              <p className="text-red-600 dark:text-red-400 text-sm mt-2">Invalid cron expression</p>
            )}
            <div className="flex gap-3 text-slate-400 dark:text-slate-600 text-xs mt-3 font-mono">
          ...
          {/* Presets */}
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-4">
            <p className="text-slate-900 dark:text-white text-sm font-medium mb-3">Common presets</p>
              ...
              {PRESETS.map(p => (
                <button key={p.value + p.label} onClick={() => setExpr(p.value)}
                  className={`flex items-center justify-between px-3 py-2 rounded-lg text-sm text-left transition-colors ${
                    expr === p.value
                      ? "bg-blue-600/15 border border-blue-500/40 text-blue-700 dark:text-blue-300"
                      : "bg-slate-100 dark:bg-slate-800/40 border border-slate-300 dark:border-slate-700/50 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
                  }`}>
                  <span>{p.label}</span>
          ...
          {isValid && nextRuns.length > 0 && (
            <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-4">
              <p className="text-slate-900 dark:text-white text-sm font-medium mb-3">Next 5 runs</p>
              <div className="space-y-2">
                {nextRuns.map((d, i) => (
                  <div key={i} className="flex items-center gap-3 text-sm">
                    <span className="text-slate-400 dark:text-slate-600 font-mono w-4">{i + 1}</span>
                    <span className="text-slate-500 dark:text-slate-400 text-xs w-8">{DAY_NAMES[d.getDay()]}</span>
          ...
          {/* Field reference */}
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-4">
            <p className="text-slate-900 dark:text-white text-sm font-medium mb-3">Field reference</p>
            <div className="space-y-2 text-xs font-mono text-slate-500 dark:text-slate-400">
              ...
              <div className="pt-2 border-t border-slate-200 dark:border-slate-800 space-y-1">
                <div className="flex gap-4"><span className="w-8 text-blue-600 dark:text-blue-400">*</span><span>Any value</span></div>
                <div className="flex gap-4"><span className="w-8 text-blue-600 dark:text-blue-400">,</span><span>Value list (e.g. 1,3,5)</span></div>
                <div className="flex gap-4"><span className="w-8 text-blue-600 dark:text-blue-400">-</span><span>Range (e.g. 1-5)</span></div>
                <div className="flex gap-4"><span className="w-8 text-blue-600 dark:text-blue-400">/</span><span>Step (e.g. */15)</span></div>
              </div>
            </div>
          </div>
        </div>
