...
const RECORD_COLORS: { [k: string]: string } = {
  A: "bg-blue-500/20 text-blue-600 dark:text-blue-400 border-blue-500/40",
  AAAA: "bg-purple-500/20 text-purple-600 dark:text-purple-400 border-purple-500/40",
  CNAME: "bg-cyan-500/20 text-cyan-600 dark:text-cyan-400 border-cyan-500/40",
  MX: "bg-orange-500/20 text-orange-600 dark:text-orange-400 border-orange-500/40",
  NS: "bg-green-500/20 text-green-600 dark:text-green-400 border-green-500/40",
  TXT: "bg-yellow-500/20 text-yellow-600 dark:text-yellow-400 border-yellow-500/40",
  SOA: "bg-slate-500/20 text-slate-500 dark:text-slate-400 border-slate-500/40",
};

function RecordTable({ type, records }: { type: string; records: DnsRecord[] }) {
  const color = RECORD_COLORS[type] ?? "bg-slate-500/20 text-slate-500 dark:text-slate-400 border-slate-500/40";
  return (
    <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 dark:bg-slate-800/40 border-b border-slate-300 dark:border-slate-700/40">
      ...
      <div className="divide-y divide-slate-200 dark:divide-slate-800/40">
        {records.map((r, i) => (
          <div key={i} className="px-4 py-2.5 flex gap-4 items-start">
            <code className="text-slate-800 dark:text-slate-200 text-sm font-mono break-all flex-1">{r.value}</code>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">DNS Lookup</h1>
        ...
        {loading && (
          <div className="mt-5 space-y-3">
            {[1, 2, 3].map(i => (
              <div key={i} className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-4 space-y-2 animate-pulse">
        ...
        {result && !loading && (
          <div className="mt-5 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-slate-500 dark:text-slate-400 text-sm">Results for <span className="text-slate-900 dark:text-white font-medium">{result.domain}</span></p>
