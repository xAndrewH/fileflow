  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-xl mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Currency Converter</h1>
        <p className="text-slate-500 text-sm mb-8">
          Live exchange rates.{lastUpdated && <span className="text-slate-400 dark:text-slate-600"> Updated {lastUpdated}.</span>}
        </p>

        {loading && <div className="text-center py-12 text-slate-500 text-sm">Loading exchange rates…</div>}
        {error && <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800/60 rounded-xl p-4 text-red-600 dark:text-red-400 text-sm">{error}</div>}
        ...
        {!loading && !error && rates && (
          <div className="space-y-4">
            <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-5 space-y-4">
              {/* Amount */}
              <div>
                <label className="text-slate-500 dark:text-slate-400 text-xs mb-1.5 block">Amount</label>
                <input
                  type="number"
                  value={amount}
                  onChange={e => setAmount(e.target.value)}
                  className="w-full bg-slate-100 dark:bg-slate-800/60 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2.5 text-slate-900 dark:text-white text-lg font-mono focus:outline-none focus:border-blue-500"
                  min="0"
                />
              </div>
              ...
              <div className="flex items-end gap-3">
                <div className="flex-1">
                  <label className="text-slate-500 dark:text-slate-400 text-xs mb-1.5 block">From</label>
                  <select
                    value={from}
                    onChange={e => setFrom(e.target.value)}
                    className="w-full bg-slate-100 dark:bg-slate-800/60 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2.5 text-slate-900 dark:text-white text-sm focus:outline-none focus:border-blue-500">
                    {currencies.map(c => (
                ...
                <button onClick={swap}
                  className="mb-0.5 px-3 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg transition-colors text-sm">
                  ⇄
                </button>
                <div className="flex-1">
                  <label className="text-slate-500 dark:text-slate-400 text-xs mb-1.5 block">To</label>
                  <select
                    value={to}
              ...
              {result && (
                <div className="bg-slate-100 dark:bg-slate-800/60 border border-slate-300 dark:border-slate-700/60 rounded-xl p-4 text-center">
                  <p className="text-slate-500 dark:text-slate-400 text-sm mb-1">{amount} {from} ({fromName}) =</p>
                  <p className="text-slate-900 dark:text-white text-3xl font-bold">{result}</p>
                  <p className="text-slate-500 dark:text-slate-400 text-base mt-1">{to}: {toName}</p>
            ...
            <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 rounded-xl p-4">
              <p className="text-slate-900 dark:text-white text-sm font-medium mb-3">Quick conversions from {from}</p>
                ...
                {POPULAR.filter(c => c !== from).slice(0, 12).map(c => {
                  ...
                  return (
                    <button
                      key={c}
                      onClick={() => setTo(c)}
                      className={`w-full flex justify-between items-center px-3 py-2 rounded-lg text-xs transition-colors ${to === c ? "bg-blue-600/30 border border-blue-500/40 text-blue-700 dark:text-blue-300" : "bg-slate-100 dark:bg-slate-800/60 border border-slate-300 dark:border-slate-700/50 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}>
