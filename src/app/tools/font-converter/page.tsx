...
// WOFF2 is intentionally not offered here: the wawoff2 WASM module used for WOFF2
// compress/decompress does not currently initialize correctly in this app's browser
// bundle (a pre-existing issue in src/lib/font-client.ts, not specific to this page).
const FONT_EXT_RE = /\.(ttf|otf|woff)$/i;
...
export default function FontConverterPage() {
  ...
  const [format, setFormat]     = useState<Fmt>("woff");
  ...
  const addFiles = useCallback((files: File[]) => {
    const valid = files.filter((f) => FONT_EXT_RE.test(f.name));
    if (valid.length === 0) { setError("Please select font files (TTF, OTF, WOFF)."); return; }
    setError("");
    setFonts((prev) => [
      ...prev,
  ...
  const remove = (id: string) => setFonts((prev) => {
  ...
  const clearAll = () => {
    fonts.forEach((i) => { if (i.resultUrl) URL.revokeObjectURL(i.resultUrl); });
    setFonts([]);
    setError("");
  };

  const convertAll = async () => {
    console.log("[DEBUG] convertAll called, fonts.length=", fonts.length);
    if (fonts.length === 0) return;
    setProcessing(true);
    setError("");

    fonts.forEach((i) => { if (i.resultUrl) URL.revokeObjectURL(i.resultUrl); });
    setFonts((prev) => prev.map((i) => ({ ...i, status: "pending", resultUrl: undefined, resultSize: undefined, error: undefined })));

    console.log("[DEBUG] before dynamic import of font-client");
    const { convertFontClient } = await import("@/lib/font-client");
    console.log("[DEBUG] after dynamic import of font-client", typeof convertFontClient);
    for (const item of fonts) {
      console.log("[DEBUG] converting item", item.file.name, "to", fmt.id);
      try {
        const blob = await convertFontClient(item.file, fmt.id);
        console.log("[DEBUG] conversion resolved, blob size", blob.size);
        const url = URL.createObjectURL(blob);
        setFonts((prev) => prev.map((i) => i.id === item.id ? { ...i, status: "done", resultUrl: url, resultSize: blob.size } : i));
      } catch (e) {
        setFonts((prev) => prev.map((i) => i.id === item.id ? { ...i, status: "error", error: (e as Error).message } : i));
      }
    }
    setProcessing(false);
  };

  const downloadOne = (item: FontFile) => {
    if (!item.resultUrl) return;
    const a = document.createElement("a");
    a.href = item.resultUrl;
    a.download = `${baseName(item.file.name)}.${fmt.id}`;
    a.click();
  };

  const downloadAll = async () => {
    const done = fonts.filter((i) => i.status === "done" && i.resultUrl);
    ...
    for (const item of done) {
      ...
      zip.file(`${baseName(item.file.name)}.${fmt.id}`, blob);
    }
    const blob = await zip.generateAsync({ type: "blob" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `converted-${fmt.id}.zip`;
    a.click();
    URL.revokeObjectURL(a.href);
  };

  const doneCount = fonts.filter((i) => i.status === "done").length;
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">Font Converter</h1>
          <p className="text-slate-500 text-sm">Convert fonts between TTF, OTF, and WOFF, in bulk, entirely in your browser.</p>
        </div>
        ...
        >
          ...
          <div className="text-center">
            <p className="font-medium text-sm">Drop fonts here</p>
            <p className="text-xs opacity-60 mt-0.5">or click to browse — TTF, OTF, WOFF</p>
          </div>
          <input
            ref={inputRef}
            type="file"
          ...
          <div className="flex gap-2">
            ...
            >
              {processing ? (
                ...
                `Convert ${fonts.length} font${fonts.length !== 1 ? "s" : ""} to ${fmt.label}`
              )}
            </button>
            {doneCount > 0 && (
        ...
        <RelatedTools current="/tools/font-converter" />
      </div>
    </div>
  );
}
